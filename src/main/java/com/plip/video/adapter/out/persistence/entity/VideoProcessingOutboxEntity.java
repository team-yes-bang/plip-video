...
@Entity
@Table(
		name = "video_processing_outbox",
		uniqueConstraints = @UniqueConstraint(
				name = "uk_outbox_video_event",
				columnNames = {"video_uuid", "event_type"}
		)
)
@NoArgsConstructor(access = AccessLevel.PROTECTED)
public class VideoProcessingOutboxEntity {
	...
	private VideoProcessingOutboxEventType eventType;
	...
	public static VideoProcessingOutboxEntity pending(
			UUID videoUuid,
			VideoProcessingOutboxEventType eventType,
			String payloadJson,
			LocalDateTime now
	) {
		VideoProcessingOutboxEntity entity = new VideoProcessingOutboxEntity();
